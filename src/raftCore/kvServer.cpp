...
void KvServer::Get(const raftKVRpcProctoc::GetArgs *args, raftKVRpcProctoc::GetReply *reply) {
  ...
  op.RequestId = args->requestid(); //代表某一个请求id，用于后续判断是否重复？
  ...
  if (!chForRaftIndex->timeOutPop(CONSENSUS_TIMEOUT, &raftCommitOp)) { //这里chForRaftIndex就是对应的lockqueue
...
void KvServer::PutAppend(const raftKVRpcProctoc::PutAppendArgs *args, raftKVRpcProctoc::PutAppendReply *reply) {
  ...
  m_raftNode->Start(op, &raftIndex, &_, &isleader); //传入地址，设置正常的raftindex，term
  ...
  m_mtx.unlock();  //直接解锁，等待任务执行完成，不能一直拿锁等待，因为后续的访问是该map里raftindex对应的队列，不是map无须加锁
...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port) : m_skipList(6) {
  ...
  std::thread t([this, port]() -> void {
    ...
    provider.NotifyService(this); //注册kv服务器，针对客户端
    provider.NotifyService( 
        this->m_raftNode.get());  // 注册rpc节点，针对其他节点，todo：这里获取了原始指针，后面检查一下有没有泄露的问题 或者 shareptr释放的问题
    // 启动一个rpc服务发布节点   Run以后，进程进入阻塞状态，等待远程的rpc调用请求,所以需要新线程，并分离
    provider.Run(m_me, port); //在这里写入ip+port
  });
  t.detach();
  ...
  for (int i = 0; i < ipPortVt.size(); ++i) {
    ...
    auto *rpc = new RaftRpcUtil(otherNodeIp, otherNodePort); //远程连接
