...
class Raft : public raftRpcProctoc::raftRpc {
 private:
  std::mutex m_mtx;
  std::vector<std::shared_ptr<RaftRpcUtil>> m_peers; //每个元素封装了针对一个远端 Raft 节点的 RPC 调用逻辑，用于节点间通信。
  std::shared_ptr<Persister> m_persister; //持久化，负责写入磁盘
  int m_me; //本节点ID
  int m_currentTerm; //本任期号
  int m_votedFor; //所投ID
  std::vector<raftRpcProctoc::LogEntry> m_logs;  //// 日志条目数组，包含了状态机要执行的指令集，以及收到领导时的任期号
                                                 // 这两个状态所有结点都在维护，易失
  int m_commitIndex; // 已提交日志最大索引
 ...
 public:
  void AppendEntries1(const raftRpcProctoc::AppendEntriesArgs *args, raftRpcProctoc::AppendEntriesReply *reply);  //处理leader发过来的心跳，日志
  void applierTicker(); 
