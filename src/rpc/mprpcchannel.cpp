...
bool MprpcChannel::newConnect(const char* ip, uint16_t port, string* errMsg) {
  ...
  struct sockaddr_in server_addr; //表示 IPv4 地址信息 的结构体
  server_addr.sin_family = AF_INET;
  server_addr.sin_port = htons(port);
  server_addr.sin_addr.s_addr = inet_addr(ip);
  // 连接rpc服务节点
  if (-1 == connect(clientfd, (struct sockaddr*)&server_addr, sizeof(server_addr))) { //(struct sockaddr*)&server_addr类型转换符合传参
    close(clientfd);
    char errtxt[512] = {0};
