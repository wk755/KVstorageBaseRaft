...
std::string Clerk::Get(std::string key) {
  ...
  //确实会存在一直死循环，这种情况可以设置一个最大重试次数，一般是网络一直连不上远端rpc接口，或者找不到leader（可能性极低）
  while (true) {
    raftKVRpcProctoc::GetReply reply;
    bool ok = m_servers[server]->Get(&args, &reply); //从stub出发，发送给远端rpc接口，实际上就是对stub做了一个封装而已
    if (!ok ||
        reply.err() ==
...
void Clerk::Init(std::string configFileName) {
  ...
  MprpcConfig config;
  config.LoadConfigFile(configFileName.c_str()); //将m_configMap填满对应的ip，port
  ...
  for (const auto& item : ipPortVt) {
    ...
    auto* rpc = new raftServerRpcUtil(ip, port); //相当于创建了一个代理stub，传入对应ip，port，每个raft对应一个stub
    m_servers.push_back(std::shared_ptr<raftServerRpcUtil>(rpc)); //就是智能指针的构造，前面《》里对应类型，后面（）里面是构造函数所传参数，即一个指针
  }
}
