...
triggerEvent(Event event) {
  ...
  events = (Event)(events & ~event); //后面相当于全部取反，然后一与，移除事件
...
IOManager::IOManager(size_t threads, bool use_caller, const std::string &name) : Scheduler(threads, use_caller, name) {
  ...
  event.data.fd = tickleFds_[0]; //这个本身就是fd，而ret本身是一个返回值，表示操作是否成功？
  // 边缘触发，设置非阻塞
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
  ...
  int op = fd_ctx->events ? EPOLL_CTL_MOD : EPOLL_CTL_ADD; //判断操作，是修改还是添加？取决于原来有没有注册事件
  epoll_event epevent;
  ...
  // 赋值fd对应的event事件的EventContext，用户态的操作,先是改变fd_ctx里面注册（感兴趣）事件
  fd_ctx->events = (Event)(fd_ctx->events | event);
  //以下是改变fd_ctx里面具体发生事件，就是如果fd就绪，改怎么做？而不是普通的读或者怎么样，实际上相当于将发生行为包装成某个函数了
...
bool IOManager::delEvent(int fd, Event event) {
  ...
  --pendingEventCnt_;
  //这里fd_ctx里面eventcontext事件上下文本来就是fd_ctx的一部分，是成员变量，不是read就是write，不能删除，只能置空
...
//动态类型转换，iomanager继承自scheduler；scheduler是多态类型，至少有一个虚函数
IOManager *IOManager::GetThis() { return dynamic_cast<IOManager *>(Scheduler::GetThis()); }

// 通知调度器有任务到来，这个是不涉及io（socket）的事件，也就是epoll里注册的并没有类似事件
//因此需要我们需要一个管道来通知有新任务到来，而这个管道的读端作为一个socket被注册到epollfd中
...
void IOManager::idle() {
  ...
  while (true) {
    ...
    listExpiredCb(cbs); //将所有已经到期（超时）的定时器对应的回调函数收集到 cbs 中
    if (!cbs.empty()) {
    ...
    for (int i = 0; i < ret; i++) {
      ...
      // 错误事件 or 挂起事件(对端关闭)，这个返回的events可能是其中一种，也可能是多种
      if (event.events & (EPOLLERR | EPOLLHUP)) {
        std::cout << "error events" << std::endl;
        event.events |= (EPOLLIN | EPOLLOUT) & fd_ctx->events; //就是一个包含读或者写的新事件掩码
      }
      // 实际发生的事件类型
    ...
    // 检测，拿取新任务去调度
    Fiber::ptr cur = Fiber::GetThis(); //就是 idlefiber ，在scheduler::run（）里创建的
    auto raw_ptr = cur.get();
    cur.reset(); //释放临时引用，因为原本引用是在run()函数里面创建的fiber协程，这里一直保留着那调度协程里就无法释放
