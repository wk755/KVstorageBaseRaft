...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string &name) {
  ...
  // use_caller:是否将当前线程也作为被调度线程,本身是调度线程，看是否被当作被调度线程
  if (use_caller) {
    std::cout << LOG_HEAD << "current thread as called thread" << std::endl;
    // 总线程数减1
    --threads;
    // 初始化caller线程的主协程
    Fiber::GetThis(); //实际是分配一块空间用为主协程
    ...
    cur_scheduler = this;
    // 初始化当前线程的调度协程 （该线程不会被调度器），调度结束后，返回主协程
    //指向调度协程的智能指针，这个fiber构造参数，cb，stacksize（0为默认），是否参与调度
    ...
    cur_scheduler_fiber = rootFiber_.get(); //协程级调度器
...
void Scheduler::start() {
  ...
  for (size_t i = 0; i < threadCnt_; i++) {
    //线程池里的每一条线程都是在调用 同一个 Scheduler 实例的 run() 方法，共享那个实例内部的任务队列、锁、计数器等资源。
...
void Scheduler::run() {
  ...
  setThis(); //设置cur_scheduler
  if (GetThreadId() != rootThread_) {
    // 如果当前线程不是caller线程，则初始化该线程的调度协程
    cur_scheduler_fiber = Fiber::GetThis().get(); //返回裸指针
  }

  // 创建idle协程
  Fiber::ptr idleFiber(new Fiber(std::bind(&Scheduler::idle, this))); //当任务队列为空时，切到它去做“空闲行为”，this是对象实例，可以是iomanager
  Fiber::ptr cbFiber; //一个声明而已

  SchedulerTask task; //cb，fiber，thread
  while (true) {
    task.reset();
    // 是否通知其他线程进行任务调度
...
void Scheduler::idle() {
  while (!stopping()) {
    Fiber::GetThis()->yield(); //正在运行的fiber切回调度主协程
  }
}
...
void Scheduler::stop() {
  ...
  // stop指令只能由caller线程发起
  //其实是统一地把“发起 stop()”的权限，永久锁定在那个最初调用 start() 的主线程身上
  if (isUseCaller_) { 
    //这种情况是caller线程被当作工作线程，设置了cur_thread
    CondPanic(GetThis() == this, "cur thread is not caller thread");
  } else {
    //这种情况主线程（即创建scheduler线程）并没有设置cur_thread
