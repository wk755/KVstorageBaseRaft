...
void *Thread::run(void *arg) {
  Thread *thread = (Thread *)arg; //转换类型，就是上面的this
  cur_thread = thread;
...
void Thread::join() {
  if (thread_) {
    int rt = pthread_join(thread_, nullptr); //pthread_join 会让当前线程阻塞，直到 thread_ 对应的那个后台线程执行完毕并退出。
    if (rt) {
      std::cout << "pthread_join error,name:" << name_ << std::endl;
      throw std::logic_error("pthread_join");
    }
    thread_ = 0; //表示这条线程已经被 join 并且资源已回收。
  }
}
