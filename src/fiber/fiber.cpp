...
//这里仅仅用了malloc，free，实际上可以用线程池，栈分配器
...
// only for GetThis
//构造线程启动时的 “主协程”，代表线程自身执行上下文。
Fiber::Fiber() {
  SetThis(this); //当前正在线程运行的协程
  state_ = RUNNING;
  CondPanic(getcontext(&ctx_) == 0, "getcontext error"); //保存当前上下文
  ++fiber_count;
  id_ = cur_fiber_id++;
...
// 有参构造，并为新的子协程创建栈空间
//为这个协程分配独立的栈空间；
//用 makecontext 将它的入口指向静态函数 MainFunc，在里面会执行用户传入的 cb_。
...
void Fiber::yield() {
  CondPanic(state_ == TERM || state_ == RUNNING, "state error");
  SetThis(cur_thread_fiber.get()); //调度协程
  if (state_ != TERM) {
    state_ = READY;
  }
  if (isRunInScheduler_) {
...
void Fiber::reset(std::function<void()> cb) {
  ...
  cb_ = cb;
  CondPanic(0 == getcontext(&ctx_), "getcontext failed"); //获取此时此刻的ctx_
  ctx_.uc_link = nullptr;  //uc_link 指定当这个上下文运行结束后要自动跳转到哪个上下文
  ctx_.uc_stack.ss_sp = stack_ptr; //使用该用户栈
  ctx_.uc_stack.ss_size = stackSize_;  //指定栈大小
