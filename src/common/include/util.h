...
class LockQueue {
 public:
  ...
  {
    std::unique_lock<std::mutex> lock(m_mutex); //这里加锁仍然是因为多个线程使用同一个队列，对于生-消模型很正常，因为一个消费pop，一个生成会push
    ...
    while (m_queue.empty()) {
      // 如果已经超时了，就返回一个空对象
      if (m_condvariable.wait_until(lock, timeout_time) == std::cv_status::timeout) { //条件变量避免忙等待，后面这个本身就是超时标志
        return false;
      } else {
        continue;
      }
